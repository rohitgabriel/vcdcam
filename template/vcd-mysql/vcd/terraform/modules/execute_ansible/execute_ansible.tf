...
EOF
    ...
    destination = "/tmp/ansible-playbook-host-mysql"
  }
...
EOF
  ...
  provisioner "remote-exec" {
    inline = [
      "sleep 180 && cd ${var.playbook_location} && ansible-playbook -i \"/tmp/ansible-playbook-host-mysql\" main.yml -e \"ansible_python_interpreter=/usr/bin/python3\"",
    ]
  }
}
